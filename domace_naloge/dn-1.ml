...
let sep_concat x l = 
  let dodaj_locilo sez =
    sez @ [x]
  in
  x :: List.concat_map dodaj_locilo l
...
let partition k sez = 
  let rec aux sez acc =
    if sez = [] then acc
    else if List.length(sez) < k then
      sez :: acc
    else
      aux (drop (k) sez) ((take k sez) :: acc)
  in
  List.rev (aux sez [])
...
let kompozitum p q = 
  let element i =
    List.nth p (List.nth q i)
  in
  let indeksi = List.rev (n_list (List.length p)) in
  List.map element indeksi
...
let inverz p = 
  let element i x =
    (i, x)
  in
  let sez_pari = List.mapi element p in
  let urejeni_pari = List.sort sort_by_el sez_pari in
  let get_el (i, x) =
    i
  in
  List.map get_el urejeni_pari
...
let cikli permutacija = 
  let rec po_perm i obiskani sez_perm =

    let rec en_cikel j cikel obisk =
      if List.mem j obisk then
        if cikel = [] then
          po_perm (i + 1) (List.append obisk obiskani) sez_perm
        else
          po_perm (i + 1) (List.append obisk obiskani) ((List.rev cikel) :: sez_perm)
      else
        en_cikel (List.nth permutacija j) (j :: cikel) (j :: obisk)
    in

    if i < List.length permutacija then
      en_cikel i [] obiskani
    else 
      sez_perm
  in
